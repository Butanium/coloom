...
- **Shared-active-path policy** — Tapestry never had to decide whether navigation re-routes a path someone *else* is reading. coloom does (flagged in §2). **→ Resolved 2026-06-09: per-participant named cursors, movable by anyone, replace the active path (see `docs/PLAN.md`).**
