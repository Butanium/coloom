...
root, bookmarks, a split node, and two cursors (clement + claude). Prints the weave id.
...
def main() -> None:
    ...
    wid = c.post(
        "/weaves", json={"title": args.title, "description": "seeded for UI dev/testing"}
    ).json()["id"]
    ...
    kids = gen(wid, root["id"], "clement", n=3)
    grandkids = gen(wid, kids[0]["id"], "claude", n=3)
    gen(wid, kids[1]["id"], "claude", n=2)
    # depth 3 under the first grandkid, claude follows his generation
    deep = c.post(
        f"/weaves/{wid}/gen",
        json={"node_id": grandkids[0]["id"], "cursor": "claude", "move_cursor": True,
    ...
    gen(wid, second_root["id"], "clement", n=2)
    ...
    # clement's cursor onto the first branch
    c.put(
        f"/weaves/{wid}/cursors/clement",
        json={"node_id": kids[0]["id"], "moved_by": "clement"},
    )
